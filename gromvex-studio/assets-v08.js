(() => {
  ...
  function objectUrlFor(asset) {
    if (asset?.contentUrl) return server?.resolveUrl?.(asset.contentUrl) || asset.contentUrl;
  ...
  function bindEvents() {
    ...
    window.addEventListener('gromvex-server-mode', () => {
      ...
      refreshAllAssetUi().catch(console.error);
    });
